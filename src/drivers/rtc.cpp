...
void set_rtc_time(int16_t hour, int16_t min, int16_t sec, int16_t week_num) {
    ...
    if (hour < 0 || hour > 23) {
        Serial.println("[RTC] set_rtc_time: invalid hour");
        return;
    }
    if (min < 0 || min > 59) {
        Serial.println("[RTC] set_rtc_time: invalid minute");
        return;
    }
    if (sec < 0 || sec > 59) {
        Serial.println("[RTC] set_rtc_time: invalid second");
        return;
    }
    if (week_num < 0 || week_num > 6) {
        Serial.println("[RTC] set_rtc_time: invalid weekday (0-6)");
        return;
    }
